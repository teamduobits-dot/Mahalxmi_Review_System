...
const STEP_ITEMS = [
  ...
  },
  {
    ...
    subtitle: 'Review screenshot, same name and Order ID last 4',
    icon: 'burger',
...
export default function CustomerForm() {
  ...
  const animateScrollTo = (targetY, duration = 760) => {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden surface-warm">
      ...
      <main className="relative z-10 mx-auto w-full max-w-3xl px-4 pb-28 pt-safe sm:px-6">
        <section className="mx-auto max-w-2xl pt-4 text-center">
          <motion.div
            initial={{ opacity: 0, y: 12 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.35 }}
            className="inline-flex items-center gap-2 rounded-full border border-white/80 bg-white/90 px-4 py-2 text-xs font-extrabold text-cocoa-900 shadow-soft backdrop-blur-sm"
          >
          ...
          <div className="relative mx-auto mt-5 max-w-xl">
            ...
            <motion.div
              ...
              className="rounded-[2rem] border border-white/75 bg-white/72 px-4 py-5 shadow-card backdrop-blur-sm"
            >
              <div className="mx-auto mb-3 flex w-fit items-center gap-2 rounded-full bg-brand-50 px-3 py-1.5 text-[11px] font-black uppercase tracking-[0.16em] text-brand-700">
          ...
          <motion.p
            ...
            className="mx-auto mt-4 max-w-xl text-sm font-medium leading-7 text-cocoa-600 sm:text-base"
          >
          ...
          <motion.div
            ...
            className="mx-auto mt-4 max-w-xl rounded-[1.6rem] border border-amber-200 bg-amber-50/90 px-4 py-3 text-left shadow-soft"
          >
            <p className="text-[11px] font-black uppercase tracking-[0.18em] text-amber-700">Important for cashback</p>
        ...
        {!settings.campaignActive ? (
          <SectionCard className="mx-auto mt-8 max-w-2xl text-center">
            <div className="text-5xl">⏸️</div>
            <h2 className="mt-4 font-display text-2xl font-extrabold text-cocoa-900">Cashback is paused</h2>
            <p className="mt-2 text-sm font-medium text-cocoa-500">{settings.pauseMessage}</p>
          </SectionCard>
        ) : (
          <>
            <SectionCard ref={stepsRef} className="mx-auto mt-7 max-w-2xl p-4 sm:p-5">
              <div className="mb-4 text-center">
                <p className="text-[11px] font-black uppercase tracking-[0.24em] text-brand-600">How it works</p>
                <h2 className="mt-2 font-display text-2xl font-extrabold text-cocoa-950">3 easy steps</h2>
                <p className="mt-1 text-sm font-medium text-cocoa-500">Quick, clear and easy on mobile.</p>
              </div>

              <div className="grid justify-items-center gap-3 sm:grid-cols-3">
                {STEP_ITEMS.map((step, index) => (
                  <motion.div
                    ...
                    className={`relative w-full max-w-sm overflow-hidden rounded-[1.85rem] border border-white bg-gradient-to-b ${step.tint} p-4 text-center shadow-soft`}
                  ...
                  >
                    ...
                    <div className="relative mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-white shadow-soft">
                      <FoodIcon type={step.icon} size={30} />
                    </div>
                    <span className="relative mt-3 inline-flex h-10 w-10 items-center justify-center rounded-full bg-brand-500 text-sm font-black text-white shadow-soft">
                      {step.number}
                    </span>
                    <p className="relative mt-3 text-[1rem] font-extrabold leading-6 text-cocoa-900">{step.title}</p>
                    <p className="relative mt-1 text-xs font-medium leading-5 text-cocoa-500">{step.subtitle}</p>
                  </motion.div>
                ))}
              </div>
            </SectionCard>
            ...
            {success ? (
              <div className="mx-auto max-w-2xl">
                <Success reference={success.reference} note={settings.successNote} onReset={() => setSuccess(null)} />
              </div>
            ) : null}

            <SectionCard ref={formRef} className="mx-auto mt-6 max-w-2xl p-4 sm:p-6">
              <div className="flex items-start gap-3">
                <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-brand-50 text-brand-600 shadow-soft">
                  <Upload size={19} />
                </span>
                <div>
                  <h2 className="font-display text-xl font-extrabold text-cocoa-950">Claim your cashback</h2>
                  <p className="mt-1 text-sm font-medium leading-6 text-cocoa-500">
              ...
              <div className="mt-5 space-y-5">
                <div className="rounded-[1.75rem] border border-cocoa-100 bg-cream-50/85 p-4 shadow-soft">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    <div className="text-center sm:text-left">
                      <p className="text-[11px] font-black uppercase tracking-wider text-cocoa-500">Step 1 · Review screenshot *</p>
                      <p className="mt-1 text-xs font-medium leading-5 text-cocoa-500">
                  ...
                  <p className="mt-2 text-[11px] font-medium leading-5 text-cocoa-400">
                  ...
                  {touched.reviewScreenshot && !validations.screenshotOk ? <p className="mt-2 text-xs font-semibold text-red-600">Review screenshot is required.</p> : null}
                </div>

                <div className="grid gap-4 sm:grid-cols-2">
                  <div>
                    <label className="text-[11px] font-black uppercase tracking-wider text-cocoa-500">Step 2 · Name used for the order *</label>
                    <div className="relative mt-1.5">
                      ...
                      <input
                        ...
                        className={`w-full rounded-2xl border-2 bg-white py-3.5 pl-11 pr-4 text-sm font-semibold outline-none transition ${touched.customerName && !validations.nameOk ? 'border-red-300 ring-4 ring-red-50' : 'border-cocoa-200/80 focus:border-brand-400 focus:ring-4 focus:ring-brand-100'}`}
                      />
                    </div>
                    <p className="mt-1.5 text-[11px] font-medium leading-5 text-cocoa-400">Use the same customer name shown on your order in Swiggy or Toing.</p>
                    {touched.customerName && !validations.nameOk ? <p className="mt-1.5 text-xs font-semibold text-red-600">Please enter the order name.</p> : null}
                  </div>

                  <div>
                    <label className="text-[11px] font-black uppercase tracking-wider text-cocoa-500">Step 2 · Order ID last 4 digits *</label>
                    <input
                      ...
                      className={`mt-1.5 w-full rounded-2xl border-2 bg-white px-4 py-3.5 text-center text-sm font-semibold tracking-[0.28em] outline-none transition ${touched.orderLast4 && !validations.last4Ok ? 'border-red-300 ring-4 ring-red-50' : 'border-cocoa-200/80 focus:border-brand-400 focus:ring-4 focus:ring-brand-100'}`}
                    />
                    <p className="mt-1.5 text-[11px] font-medium leading-5 text-cocoa-400">You can find it on the package sticker/label or in your ordering app order details.</p>
                    {touched.orderLast4 && !validations.last4Ok ? <p className="mt-1.5 text-xs font-semibold text-red-600">Please enter exactly 4 digits.</p> : null}
                  </div>
                </div>

                <div>
                  <label className="text-[11px] font-black uppercase tracking-wider text-cocoa-500">Step 3 · Receive cashback via *</label>
                  <div className="mt-2 grid gap-3 sm:grid-cols-2">
                    <button
                      type="button"
                      onClick={() => handlePayoutMethod('upi')}
                      className={`rounded-[1.7rem] border-2 p-4 text-left transition ${form.payoutMethod === 'upi' ? 'border-brand-500 bg-brand-50 shadow-pop' : 'border-cocoa-200 bg-white hover:border-brand-300'}`}
                    >
                      <span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-brand-500/10 text-brand-600">
                        <CreditCard size={18} />
                      </span>
                      <p className="mt-3 text-sm font-extrabold text-cocoa-900">UPI ID</p>
                      <p className="text-xs font-medium text-cocoa-400">Best for quick transfer</p>
                    </button>
                    <button
                      type="button"
                      onClick={() => handlePayoutMethod('qr')}
                      className={`rounded-[1.7rem] border-2 p-4 text-left transition ${form.payoutMethod === 'qr' ? 'border-brand-500 bg-brand-50 shadow-pop' : 'border-cocoa-200 bg-white hover:border-brand-300'}`}
                    >
                      <span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-gold-500/10 text-gold-600">
                        <QrCode size={18} />
                      </span>
                      <p className="mt-3 text-sm font-extrabold text-cocoa-900">UPI QR</p>
                      <p className="text-xs font-medium text-cocoa-400">Upload only your QR image</p>
                    </button>
                  </div>
                </div>
                ...
                {form.payoutMethod === 'upi' ? (
                  <div>
                    <label className="text-[11px] font-black uppercase tracking-wider text-cocoa-500">UPI ID *</label>
                    <input
                      value={form.upiId}
                      onChange={(event) => setField({ upiId: event.target.value })}
                      onBlur={() => setTouched((current) => ({ ...current, upi: true }))}
                      placeholder="yourname@upi"
                      className={`mt-1.5 w-full rounded-2xl border-2 bg-white px-4 py-3.5 text-sm font-semibold outline-none transition ${touched.upi && !validations.upiBaseOk ? 'border-red-300 ring-4 ring-red-50' : 'border-cocoa-200/80 focus:border-brand-400 focus:ring-4 focus:ring-brand-100'}`}
                    />
                    <p className="mt-1.5 text-[11px] font-medium leading-5 text-cocoa-400">Enter the UPI ID where you want to receive the cashback amount.</p>
                    {touched.upi && !validations.upiBaseOk ? <p className="mt-1.5 text-xs font-semibold text-red-600">Please enter a valid UPI ID.</p> : null}
                  </div>
                ) : (
                  <div>
                    <label className="text-[11px] font-black uppercase tracking-wider text-cocoa-500">UPI QR image *</label>
                    <p className="mt-1.5 text-[11px] font-medium leading-5 text-cocoa-400">Upload only your UPI QR image for cashback payment.</p>
                    <div className="mt-2">
                      <ImageUpload
                    ...
                    {touched.upi && !validations.upiBaseOk ? <p className="mt-1.5 text-xs font-semibold text-red-600">Please upload your UPI QR image.</p> : null}
                  </div>
                )}
                ...
                <button
                  ...
                  onClick={submit}
                  disabled={busy}
                  className={`btn-shine flex w-full items-center justify-center gap-2 rounded-[1.8rem] px-6 py-4 text-base font-extrabold text-white transition ${canSubmit ? 'bg-gradient-to-b from-brand-400 to-brand-600 shadow-pop hover:brightness-105' : 'bg-cocoa-300'} ${busy ? 'opacity-80' : ''}`}
                >
            ...
            <div className="mx-auto mt-6 flex max-w-2xl flex-col gap-3 rounded-[1.8rem] border border-white/80 bg-white/80 px-5 py-4 shadow-soft backdrop-blur-sm sm:flex-row sm:items-center sm:justify-between">
              <div>
                <p className="text-sm font-extrabold text-cocoa-900">Admin access</p>
                <p className="text-xs font-medium text-cocoa-400">Only authorised admin can open the dashboard.</p>
              </div>
              <a
                href="#/admin"
                className="inline-flex items-center justify-center gap-2 rounded-2xl border border-cocoa-200 bg-white px-4 py-3 text-sm font-bold text-cocoa-700 transition hover:border-brand-300 hover:text-brand-600"
              >
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.96 }}
              className={`fixed bottom-4 right-4 z-30 flex h-[82px] w-[82px] flex-col items-center justify-center rounded-full border text-[11px] font-black leading-none backdrop-blur-sm transition sm:bottom-6 sm:right-6 sm:h-[88px] sm:w-[88px] ${jumpButtonClass}`}
...
function OfferPill({ icon: Icon, tone, children }) {
  ...
  return (
    <div className={`inline-flex items-center gap-2 rounded-full border px-4 py-2 text-sm font-black shadow-soft ${tones[tone] || tones.brand}`}>
    ...
    <div className="relative min-h-screen overflow-hidden surface-warm">
      ...
      <main className="relative z-10 flex min-h-screen items-center justify-center px-5 pt-safe pb-safe">
        <div className="w-full max-w-sm rounded-[2rem] border border-white/80 bg-white/88 p-6 text-center shadow-card backdrop-blur-md">
          ...
          <p className="mt-2 text-sm font-semibold text-cocoa-500">Preparing your cashback form…</p>
  ...
  return (
    ...
    >
      ...
      <span className="min-w-0 flex-1 text-left">
        <span className="block text-sm font-extrabold leading-5">{label}</span>
        <span className={`mt-0.5 flex items-center gap-1 text-[11px] font-semibold ${styles.hint}`}>
